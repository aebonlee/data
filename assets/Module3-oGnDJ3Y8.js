...
> 인-셀 시각화의 목적은 "값을 읽지 않고도 패턴이 보이게" 하는 것입니다. 색·아이콘은 2~3종 이내로 절제하세요. 과하면 오히려 읽기 어려워집니다.`,contentEn:`Before building dashboards, master in-cell visualization that makes the table itself readable at a glance.
...
> Goal: make patterns visible without reading values. Keep colors/icons to 2-3 kinds.`},{title:"대시보드 기획 — KPI 정의와 레이아웃 설계",titleEn:"Dashboard Planning — Defining KPIs and Layout Design",content:`좋은 대시보드는 **잘 그린 차트의 모음**이 아니라 **의사결정을 돕는 한 장의 화면**입니다. 차트를 그리기 전에 기획이 먼저입니다.
...
> KPI는 "많이 보여주기"가 아니라 "골라 보여주기"입니다. 화면에 지표가 10개 넘으면 아무것도 강조되지 않은 것과 같습니다.`,contentEn:`A good dashboard is not a collection of charts but a single screen that supports decisions.
...
> KPIs are about "selecting", not "showing everything". More than 10 metrics emphasizes nothing.`},{title:"[실습] 피벗 차트 · 슬라이서 · 시간 표시 막대 연동",titleEn:"[Practice] Pivot Charts, Slicers & Timeline Integration",content:`피벗테이블과 슬라이서를 연결하면 클릭만으로 여러 차트가 동시에 바뀌는 **인터랙티브 대시보드**가 됩니다.
...
> 슬라이서·시간 표시 막대는 같은 데이터 모델을 쓰는 피벗에만 연결됩니다. 차트들이 함께 움직이지 않으면 "보고서 연결" 설정을 먼저 확인하세요.`,contentEn:`Connecting pivot tables with slicers turns a static report into an interactive dashboard.
...
> Slicers/timelines only connect to pivots on the same data model. If charts don't move together, check Report Connections.`},{title:"[실습] 자동 갱신되는 리포트 템플릿 만들기",titleEn:"[Practice] Building Auto-Refreshing Report Templates",content:`데이터가 늘거나 바뀔 때마다 손으로 범위를 고치는 일은 피해야 합니다. **동적 범위 + 새로고침 + 동적 참조**로 한 번 만들면 계속 쓰는 템플릿을 구성합니다.
...
> \`GETPIVOTDATA\`가 거슬리면 피벗 분석 → 옵션 → "GETPIVOTDATA 생성" 끄기로 일반 참조도 쓸 수 있습니다. 단, 자동 갱신 안정성은 \`GETPIVOTDATA\`가 더 높습니다.`,contentEn:`Build a template once and reuse it: dynamic ranges + refresh + dynamic references.
...
> You can disable GETPIVOTDATA generation in PivotTable Options, but it is the most stable for auto-refresh.`},{title:"[실습] 종합 — 부서 KPI 대시보드 완성",titleEn:"[Practice] Capstone — Completing a Department KPI Dashboard",content:`1~4단계를 모두 합쳐 **하나의 부서 KPI 모니터링 대시보드**를 완성하는 미니 프로젝트입니다. 영업부 월별 데이터를 예로 진행합니다.
...
> 막히면 "데이터 → 피벗 → 컨트롤 → 레이아웃" 순서를 지키세요. 레이아웃부터 만들면 데이터 연결이 꼬이기 쉽습니다.`,contentEn:`Combine steps 1-4 into one Department KPI monitoring dashboard. Use monthly sales data as an example.
...
> Follow Data → Pivot → Controls → Layout. Starting from layout tangles the data links.`},{title:"모듈 3 정리 & 과정 마무리",titleEn:"Module 3 Wrap-up & Course Conclusion",content:`### 모듈 3 핵심 요약
...
> 데이터 분석의 끝은 "예쁜 차트"가 아니라 "더 나은 결정"입니다. 대시보드는 그 결정을 빠르게 돕는 도구일 뿐임을 기억하세요. 수고하셨습니다.`,contentEn:`### Module 3 Summary
...
> The end of analysis is "better decisions", not "pretty charts". Well done.`}]};function c(){const{language:a}=n(),t=a==="ko";return e.jsxs(e.Fragment,{children:[e.jsx("section",{className:"page-header-ed",children:e.jsxs("div",{className:"container",children:[e.jsx("div",{className:"eyebrow",children:"Module 03"}),e.jsx("h2",{children:t?"실무형 시각화 대시보드 구축":"Building Practical Visualization Dashboards"}),e.jsx("p",{children:t?"인사이트를 한 화면에 담는 대시보드 구현 · 2.0H":"Capturing insights on a single dashboard screen · 2.0H"})]})}),e.jsx(i,{seoTitle:"실무형 시각화 대시보드 구축",seoTitleEn:"Building Practical Visualization Dashboards",seoDescription:"조건부 서식·스파크라인부터 피벗 차트·슬라이서·자동 갱신 리포트까지, 실무형 KPI 대시보드를 단계별로 구축합니다.",path:"/module3",dataFiles:[o]})]})}export{c as default};
