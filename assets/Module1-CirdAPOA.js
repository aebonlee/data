import{u as s,j as e}from"./index-BAD9s6Nr.js";import{G as i}from"./GuidePage-OcNr8x0f.js";import"./SEOHead-AuUrmHjh.js";const n={id:"module1-eda",icon:"fa-magnifying-glass-chart",title:"모듈 1 · 탐색적 데이터분석(EDA)",titleEn:"Module 1 · Exploratory Data Analysis",sections:[{title:"EDA란 무엇인가 — 목적과 실무적 가치",titleEn:"What is EDA — Purpose and Practical Value",content:`**탐색적 데이터분석(EDA, Exploratory Data Analysis)**은 통계학자 존 튜키(John Tukey)가 제안한 개념으로, 데이터를 본격적으로 모델링하기 전에 **데이터의 구조·패턴·이상치·관계를 시각화와 요약통계로 탐색하는 과정**입니다.
...
실무에서 EDA는 전체 분석 시간의 60~80%를 차지하는 경우가 많습니다. 데이터를 충분히 이해한 분석가는 더 적은 시도로 더 정확한 인사이트를 도출합니다.`,contentEn:`**Exploratory Data Analysis (EDA)**, a concept introduced by statistician John Tukey, is the process of **exploring the structure, patterns, outliers, and relationships in data using visualization and summary statistics** before formal modeling.
...
In practice, EDA often takes 60–80% of total analysis time. Skipping it means building polished conclusions on flawed data — EDA is the first line of defense against "Garbage In, Garbage Out."`},{title:"비즈니스 문제 정의와 가설 수립",titleEn:"Defining Business Problems and Building Hypotheses",content:`분석은 "데이터"가 아니라 "문제"에서 출발합니다. 막연한 고민을 **데이터로 답할 수 있는 질문**으로 바꾸는 것이 분석의 첫걸음입니다.
...
> 가설이 없는 분석은 표류합니다. "이 데이터로 무엇을 확인하려 하는가?"라는 질문에 한 문장으로 답할 수 없다면, 아직 분석을 시작할 준비가 안 된 것입니다.`,contentEn:`Analysis starts not from "data" but from a "problem." The first step is turning a vague concern into a **question answerable with data** — one that includes measurable metrics, a clear time period, and a comparison target.
...
For example, the problem "new customer churn" leads to the hypothesis "customers who didn't complete onboarding churn more," verified by comparing 30-day churn rates between completed and incomplete onboarding groups.`},{title:"지표 쪼개기(Drill-down)와 다차원 분할",titleEn:"Metric Drill-down and Multi-dimensional Splitting",content:`전체 지표 하나만 보면 "무슨 일이 일어났는지"는 알 수 있지만 "왜 그랬는지"는 알 수 없습니다. **지표 쪼개기(Drill-down)**는 큰 숫자를 구성 요소로 분해해 원인을 추적하는 핵심 기법입니다.
...
이 과정을 반복하면 "어느 차원의 어느 구간에서" 변화가 일어났는지를 단계적으로 좁혀갈 수 있습니다.`,contentEn:`Looking at a single overall metric tells you "what happened" but not "why." **Drill-down** decomposes a large number into its components to trace causes.
...
In Excel: Insert → PivotTable, drag the metric into Values, a first dimension into Rows, a second into Columns to build a cross-tab, then adjust the value field (sum/average/% of total) and add Slicers to filter quickly.`},{title:"[실습] 시계열 트렌드 · 요일별/집단별 특성 분석",titleEn:"[Practice] Time-series Trends and Group Comparison",content:`이번 실습에서는 시간에 따른 추세와 집단 간 차이를 엑셀로 직접 분석합니다. 공공데이터포털의 일별 이용 데이터나 가상의 매장 매출 데이터를 사용하면 됩니다.
...
- 특정 지점에서만 나타나는 이상한 추세가 있는가?`,contentEn:`This practice analyzes trends over time and differences between groups using Excel, with public-portal daily data or sample store-sales data.
...
**Step 3 — Group comparison:** Place customer type (new/existing) or branch in Rows, include both sum and average to see scale and efficiency together, and build a day × customer-type cross-tab to spot patterns. Always look at averages and ratios, not just totals — a large branch may have high totals but low per-customer sales.`},{title:"[실습] 상관분석 · 교차분석으로 숨은 패턴 찾기",titleEn:"[Practice] Correlation and Cross-tabulation Analysis",content:`두 변수가 함께 움직이는지(상관), 범주 간에 관계가 있는지(교차)를 살펴 숨은 패턴을 찾습니다.
...
- 교차표에서 비율 차이가 가장 큰 집단은 어디인가?`,contentEn:`This practice finds hidden patterns by checking whether two variables move together (correlation) and whether categories are related (cross-tabulation).
...
In Excel, compute \`=CORREL(B2:B100, C2:C100)\`, use Data Analysis → Correlation for a matrix, and add a scatter plot with a trendline. For categorical variables, build a PivotTable cross-tab with one variable in Rows, another in Columns, Count as values, then switch to "% of Row Total" to compare rates.`},{title:"[실습] 데이터 분포 확인 — 히스토그램 · 박스플롯",titleEn:"[Practice] Checking Distributions — Histogram and Box Plot",content:`요약통계량(평균, 합계)만 보면 데이터의 진짜 모습을 놓칩니다. 이번 실습에서는 **분포의 형태**를 직접 들여다봅니다.
...
- 집단별 박스플롯을 비교했을 때 가장 분포가 넓은(편차가 큰) 집단은 어디인가?`,contentEn:`Summary statistics like the mean hide the true shape of data. This practice examines the **distribution** directly.
...
**Box plots** show quartiles (Q1, median, Q3), the IQR, whiskers (Q1−1.5×IQR to Q3+1.5×IQR), and outliers as points. In Excel: Insert → Chart → Box and Whisker; plotting groups side by side compares distributions and outliers at once. Use \`=QUARTILE.INC(range, 1)\` and \`=MEDIAN(range)\`.`},{title:"모듈 1 정리 & 체크리스트",titleEn:"Module 1 Summary & Checklist",content:`### 핵심 요약
...
EDA로 발견한 인사이트를 **남에게 전달**하려면 시각화가 필요합니다. 다음 모듈에서는 목적에 맞는 차트 선택(비교·추세·구성·관계), 좋은 차트와 나쁜 차트의 차이, 그리고 한눈에 메시지가 전달되는 대시보드 설계 원칙을 다룹니다.`,contentEn:`### Summary
...
**Next module — Data Visualization**: choosing the right chart for comparison, trend, composition, or relationship; good vs bad charts; and dashboard design that delivers the message at a glance.`}]};function c(){const{language:a}=s(),t=a==="ko";return e.jsxs(e.Fragment,{children:[e.jsx("section",{className:"page-header-ed",children:e.jsxs("div",{className:"container",children:[e.jsx("div",{className:"eyebrow",children:"Module 01"}),e.jsx("h2",{children:t?"탐색적 데이터분석(EDA)":"Exploratory Data Analysis (EDA)"}),e.jsx("p",{children:t?"데이터를 쪼개고 파악하는 EDA 분석 기법 · 2.0H":"EDA techniques to split and understand data · 2.0H"})]})}),e.jsx(i,{seoTitle:"모듈 1 · 탐색적 데이터분석(EDA)",seoTitleEn:"Module 1 · Exploratory Data Analysis",seoDescription:"EDA의 목적, 문제 정의와 가설 수립, 지표 쪼개기, 시계열·상관·교차분석, 분포 확인까지 실습 중심으로 배우는 탐색적 데이터분석 강의안",path:"/module1",dataFiles:[n]})]})}export{c as default};
